...
Before we can register users using the NodeJS SDK, we have to add organization affiliations to the configuration files.

The fabric-ca-server is configured using a `fabric-ca-server-config.yaml` file, which is created during the `fabric-ca-server start -b admin:adminpw -d` command if it does not exist.

To add the affiliations, we'll do the following steps:

### Start the `fabric-ca-server`. 
  
  You can do this using `docker-compose -f docker-compose.yaml ca.fredrick.coderschool.vn -d`

### Copy the yaml file from the container your host laptop 

```bash
docker cp ca.fredrick.coderschool.vn:/etc/hyperledger/fabric-ca-server/fabric-ca-server-config.yaml fabric-ca-server-config.yaml
```

You can now shutdown the fabric-ca-server with `docker-compose -f docker-compose.yaml down`

### Edit the `affiliations` section

change the `affiliations` section in the `fabric-ca-server-config.yaml` to be:
...
### Mount the config file into all the fabric-ca containers

In your `docker-compose.yaml` file, mount the `fabric-ca-server-config.yaml` back into all the `fabric-ca` containers.  For example, your `ca.fredrick.coderschool.vn` service should look like this
...
Now when you start your network with `docker-compose -f docker-compose.yaml up -d`, the fabric-ca-servers should be usable with the Fabric NodeJS SDK 

# Sample `fabric-ca-server-config.yaml` file
